...
type Click = { timestamp: string; referrer: string | undefined; ip: string | undefined };
type Short = { code: string; url: string; createdAt: string; expiresAt: string; clicks: Click[] };
const codeToShort = new Map<string, Short>();
...
app.post("/shorturls", async (req: Request, res: Response) => {
  ...
  const code = shortcode ?? (() => {
    let c = generateCode();
    while (codeToShort.has(c)) c = generateCode();
    return c;
  })();
 
  if (codeToShort.has(code)) return res.status(409).json({ message: "shortcode already exists" });
 
  const now = new Date();
  const createdAt = now.toISOString();
  const expiresAt = new Date(now.getTime() + validity * 60_000).toISOString();
...
app.get("/:code", (req: Request, res: Response) => {
  ...
  if (new Date(item.expiresAt).getTime() <= Date.now()) return res.status(410).json({ message: "link expired" });
...
app.get("/shorturls/:code/stats", (req: Request, res: Response) => {
  const code = String(req.params.code || "").toLowerCase();
  const item = codeToShort.get(code);
  if (!item) return res.status(404).json({ message: "not found" });
